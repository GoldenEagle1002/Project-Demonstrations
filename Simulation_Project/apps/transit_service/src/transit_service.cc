...
public:
    ...
    /**
     * @brief Alerts user of time left on there delivery when complete.
     * @param name - desegnator of type of package.
     * @param time - amount of time left on delivery
     * @param timeOut - whether the devierly made it in time or not
     **/
    void sendDeliveryTimerStatus(const std::string& name, const int& time, const bool& timedOut) {
        JsonObject details;
        details["Name"] = name;
        details["Time"] = time;
        details["HasTimedOut"] = timedOut;
        sendEventToView("DeliveryTimer", details);
    }
...
public:
   ...
   **/
    void sendDeliveryTimerStatus(const std::string& name, const int& time, const bool& timedOut) {
        for (int i = 0; i < sessions.size(); i++) {
            static_cast<TransitService*>(sessions[i])->sendDeliveryTimerStatus(name, time, timedOut);
        }
    }
